  ...
  const submitForm = async (e) => {
    ...
    for (const field in rawMaterial) {
      if (field !== "date" && !/^[A-Za-z.\s]+$/.test(rawMaterial[field])) {
        newErrors[field] =
          `${field} cannot contian numbers or special characters.`;
        isValid = false;
      }
    }
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/createRawMaterial",
        rawMaterial,
      );
      ...
      if (error.response) {
        toast.error(
          error.response.data.msg ||
            "Failed to submit raw material. Please try again.",
          { position: "top-right" },
        );
            ...
            <div className="left-side">
              ...
              <div className="inputGroup">
                ...
                >
                  ...
                  <option value="Spices and Seasonings">
