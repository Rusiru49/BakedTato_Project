...
import { Box, Typography, Button, Container, Grid, Card, CardMedia, CardContent, CardActions, IconButton } from '@mui/material';
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ backgroundColor: 'background.default', minHeight: '100vh' }}>    
        <IconButton sx={{ position: 'absolute', top: 16, right: 16, color: 'primary.main' }}>
          <AccountCircleIcon fontSize="large" />
        </IconButton>

      
        <Box sx={{ bgcolor: 'primary.main', color: 'white', py: 12, textAlign: 'center', position: 'relative' }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.3 }}>
            <Typography variant="h5" paragraph>
              The ultimate baked potato experience. Crispy on the outside, fluffy on the inside, and loaded with flavor.
            </Typography>
          </motion.div>
        </Box>
            ...
            },
            {
              ...
              <Grid item key={index} xs={12} sm={6} md={4}>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper', boxShadow: 3 }}>
                    <CardMedia component="img" image={feature.image} alt={feature.title} sx={{ height: 200, objectFit: 'cover' }} />
