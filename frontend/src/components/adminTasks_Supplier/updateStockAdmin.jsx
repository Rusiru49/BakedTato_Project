...
const UpdateRemainingStockAdmin = () => {
  const { id } = useParams();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/update-stock-admin/${id}`, {
        currentStock: updatedStock,
        date: selectedDate.toISOString()
      });

      toast.success("Stock updated successfully!");
      navigate("/manage-stock-admin");
    } catch (err) {
      console.error(err);
    ...
    <div className="createFormRaw">
      <div className="formContainer">
        <div className="button-container">
          <Link to="/manage-stock-admin" className="backBtn">
...
export default UpdateRemainingStockAdmin;
